...
- Data: fastMRI knee dataset, multicoil challenge, gated access from https://fastmri.med.nyu.edu/ (approval time unknown); large enough it must live on CHTC `/staging`, transferred as `.tar.gz`, not bundled into `transfer_input_files` the way the smaller CIFAR-10 prior project did
